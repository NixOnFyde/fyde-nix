...
| AP6275P WiFi + firmware                                            | `working`  | NM uses wpa_supplicant backend by default (`hardware.fydetabduo.wifi.backend`); NM pinned to permanent MAC (brcmfmac rejects MAC randomization) |
...
| Hibernation (suspend-to-disk, S4)                                  | `untested` | `hardware.fydetabduo.hibernation` sets up the swapfile and resume params; just provide `resumeOffset` (see [HIBERNATION.md](HIBERNATION.md))    |
...
| WiFi backend (iwd vs wpa_supplicant)                               | `eval`     | `hardware.fydetabduo.wifi.backend` defaults to `wpa_supplicant` (better WPA3/SAE support)                                                       |
...
| Fingerprint reader (microarray/madev)                              | `partial`  | driver builds + loads; module autoloads at boot, `/dev/madev0` accessible; no open driver yet                                                   |
