...
This appends `scale <factor>` to the kanshi output definition (both portrait and landscape profiles), so scaling works in every orientation. `null` (the default) keeps the native resolution.
