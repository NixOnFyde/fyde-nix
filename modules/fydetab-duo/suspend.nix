    ...
    (lib.mkIf cfg.deepSuspend.enable {
      ...
      systemd.services."wowlan-disable" = {
        ...
        serviceConfig = {
          ...
          ExecStop = "${lib.getExe pkgs.bash} -c 'set -uo pipefail; LOG=/var/log/fydetab-suspend.log; echo \"=== resume $(date) ===\" >> $LOG; sleep 5; echo post-resume-wait-done >> $LOG; ${lib.getExe' pkgs.util-linux "rfkill"} unblock wifi >> $LOG 2>&1 || true; echo 1 > /sys/bus/pci/devices/0004:41:00.0/remove 2>> $LOG && echo removed=ok >> $LOG || echo removed=skip >> $LOG; sleep 1; GPIO23=\"\"; if [ -d /sys/class/gpio/gpio23 ]; then GPIO23=/sys/class/gpio/gpio23; elif [ -w /sys/class/gpio/export ]; then echo 23 > /sys/class/gpio/export 2>/dev/null || true; sleep 0.2; [ -d /sys/class/gpio/gpio23 ] && GPIO23=/sys/class/gpio/gpio23; fi; echo gpio=$GPIO23 >> $LOG; if [ -n \"$GPIO23\" ]; then echo out > $GPIO23/direction 2>/dev/null || true; echo 0 > $GPIO23/value 2>/dev/null || true; sleep 1; echo 1 > $GPIO23/value 2>/dev/null || true; sleep 5; echo power-cycled >> $LOG; fi; echo 1 > /sys/bus/pci/rescan 2>> $LOG || true; sleep 5; echo rescanned >> $LOG; PCI_DEV=$(ls /sys/bus/pci/devices/ | grep 41:00.0 || true); echo pci-dev=$PCI_DEV >> $LOG; if [ -n \"$PCI_DEV\" ]; then echo 0004:41:00.0 > /sys/bus/pci/drivers/brcmfmac/bind 2>> $LOG && echo bind=ok >> $LOG || echo bind=fail-already-bound >> $LOG; else echo device-not-found >> $LOG; fi; ${lib.getExe' pkgs.networkmanager "nmcli"} networking on >> $LOG 2>&1 || true; sleep 2; ${lib.getExe' pkgs.networkmanager "nmcli"} radio wifi on >> $LOG 2>&1 || true; echo \"=== resume done ===\" >> $LOG'";
        };
      };
