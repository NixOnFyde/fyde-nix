# Core options and hardware configuration for the FydeTab Duo.
#
# This file is imported by both:
#   - default.nix  (full module: hardware + desktop shell)
#   - hardware.nix (hardware-only: for custom compositor setups)
#
# It declares the common options (hardware.fydetabduo.enable, etc.) and
# configures the kernel, firmware, boot parameters, and overlays.
{
  config,
  lib,
  pkgs,
